...
import {IMultiStakingRewardsERC4626, IMultiTokenRewards} from '../../interfaces/IMultiStakingRewardsERC4626.sol';
import {IMultiTokenRewardsWithWithdrawalDelay} from '../../interfaces/IMultiTokenRewardsWithWithdrawalDelay.sol';
  ...
  IMultiTokenRewardsWithWithdrawalDelay
{
  ...
  mapping(IERC20Upgradeable => uint256) public periodFinish;

  /// @inheritdoc IMultiTokenRewards
  mapping(IERC20Upgradeable => uint256) public rewardRate;

  /// @inheritdoc IMultiTokenRewards
  uint256 public rewardsDuration;

  /// @inheritdoc IMultiTokenRewards
  mapping(IERC20Upgradeable => uint256) public lastUpdateTime;

  /// @inheritdoc IMultiTokenRewards
  mapping(IERC20Upgradeable => uint256) public rewardPerTokenStored;

  /// @inheritdoc IMultiTokenRewards
  mapping(IERC20Upgradeable => mapping(address => uint256)) public userRewardPerTokenPaid;

  /// @inheritdoc IMultiTokenRewards
  mapping(IERC20Upgradeable => mapping(address => uint256)) public rewards;
  ...
  mapping(address => uint256) internal _boostedBalances;
  ...
  /// @inheritdoc IMultiTokenRewards
  function lastTimeRewardApplicable(IERC20Upgradeable token) public view returns (uint256) {
    return Math.min(block.timestamp, periodFinish[token]);
  }

  /// @inheritdoc IMultiTokenRewards
  function rewardPerToken(IERC20Upgradeable token) external view returns (uint256) {
    return _rewardPerToken(token, _boostedTotalSupply);
  }

  /// @inheritdoc IMultiTokenRewards
  function earned(IERC20Upgradeable token, address account) public view returns (uint256) {
  ...
  /// @inheritdoc IMultiTokenRewards
  function getReward(address who, IERC20Upgradeable token) public nonReentrant {
  ...
  /// @inheritdoc IMultiTokenRewards
  function updateRewards(IERC20Upgradeable token, address who) external {
    _updateReward(token, who);
  }
  ...
  function _rewardPerToken(
    ...
    return
      ...
      (((lastTimeRewardApplicable(_token) - lastUpdateTime[_token]) * rewardRate[_token] * 1e18) /
        boostedTotalSupply_);
  }
  ...
  function _earned(
    ...
    return
      (boostedBalance_ *
        (_rewardPerToken(token_, boostedTotalSupply_) - userRewardPerTokenPaid[token_][account_])) /
      1e18 +
   ...
   */
  function _updateReward(IERC20Upgradeable token, address account) internal {
  ...
  function _calculateBoostedBalance(
    address account
  ) internal view virtual returns (uint256 boostedBalance_, uint256 boostedTotalSupply_) {
  ...
  function _getVotingPower(
    address account
  ) internal view returns (uint256 votingBalance, uint256 votingTotal) {
