...
interface IMultiTokenRewards {
  event RewardAdded(IERC20Upgradeable indexed reward, uint256 indexed amount, address caller);
  event RewardClaimed(
    IERC20Upgradeable indexed reward,
   ...
   */
  function periodFinish(IERC20Upgradeable reward) external view returns (uint256);
   ...
   */
  function rewardRate(IERC20Upgradeable reward) external view returns (uint256);
   ...
   */
  function lastUpdateTime(IERC20Upgradeable reward) external view returns (uint256);
   ...
   */
  function rewardPerTokenStored(IERC20Upgradeable reward) external view returns (uint256);
   ...
   * @param who The account for which the rewards are stored
   */
  function rewards(IERC20Upgradeable reward, address who) external view returns (uint256);

  /**
   * @notice Gets the second reward token for which the rewards are distributed
   */
  function rewardToken2() external view returns (IERC20Upgradeable);

  /**
   * @notice Gets the first reward token for which the rewards are distributed
   */
  function rewardToken1() external view returns (IERC20Upgradeable);
   ...
   */
  function lastTimeRewardApplicable(IERC20Upgradeable token) external view returns (uint256);
   ...
   */
  function earned(IERC20Upgradeable token, address account) external view returns (uint256);
   ...
   */
  function getReward(address who, IERC20Upgradeable token) external;
   ...
   */
  function notifyRewardAmount(IERC20Upgradeable token, uint256 reward) external;
